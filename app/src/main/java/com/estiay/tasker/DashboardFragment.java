...
public class DashboardFragment extends Fragment {
    ...
    //    for grading system of tasks
    private int getGrade(ArrayList<TaskModel> unfinished, ArrayList<TaskModel> finished){

        // count the size of the unfinished tasks
        int unfinishedSize = unfinished.size();
        // count the size of the finished tasks
        int finishedSize = finished.size();
        // add to get total size
        int totalSize = finishedSize + unfinishedSize;

        // divide unfinished tasks with total tasks and multiply by 100
        return (int)(((double) unfinishedSize / (double) totalSize) * 100);
    }

    // display week average score
    private double getWeekAverageScore(int[] weekListPoints){
        int total = 0;
        int size = weekListPoints.length;
        for(int point : weekListPoints){
            total+=point;
        }
